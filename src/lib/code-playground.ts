...
export function initializeCodePlayground(
  ...
  const source = root.querySelector<HTMLTextAreaElement>('[data-code-source]');
  ...
  const runButton = root.querySelector<HTMLButtonElement>('[data-code-run]');
  ...
  const renderEditor = () => {
    ...
    highlight.innerHTML = `${options.highlight(source.value)}\n`;
  ...
  const run = async () => {
    ...
    try {
      ...
      finish(
        `The browser build is unavailable.\n\n${error instanceof Error ? error.message : String(error)}`,
        'stderr'
      );
    }
  };
