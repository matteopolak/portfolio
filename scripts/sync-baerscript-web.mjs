...
if (!pointer.enabled) {
  console.log('BaerScript web bundle is not published yet; skipping sync.');
  process.exit(0);
}
...
for (const [name, value] of Object.entries({
  ...
  if (typeof value !== 'string' || !value) {
    throw new Error(`baerscript-web-release.json has no valid ${name}`);
  }
}
if (!/^[\w.-]+\/[\w.-]+$/.test(pointer.repository)) {
  throw new Error('invalid GitHub repository in baerscript-web-release.json');
}
if (
  !/^[\w.-]+$/.test(pointer.tag) ||
  !/^[\w.-]+\.tar\.gz$/.test(pointer.bundleAsset)
) {
  throw new Error('invalid BaerScript release tag or bundle asset name');
}
if (!/^[a-f0-9]{64}$/.test(pointer.sha256)) {
  throw new Error('invalid BaerScript bundle SHA-256');
}
...
try {
  ...
  if (stamp.sha256 === pointer.sha256) {
    console.log(
      `BaerScript web assets are current (${pointer.sha256.slice(0, 12)}).`
    );
    process.exit(0);
  }
} catch {
...
const temporaryRoot = await mkdtemp(join(tmpdir(), 'baerscript-web-'));
...
try {
  console.log(
    `Downloading BaerScript ${pointer.baerscriptRevision ?? 'web bundle'}…`
  );
  ...
  const entries = (await run('tar', ['-tzf', archivePath]))
    .split('\n')
    .filter(Boolean);
  if (entries.length === 0) throw new Error('BaerScript bundle is empty');
  for (const entry of entries) {
    ...
    if (
      ...
      throw new Error(`unsafe path in BaerScript bundle: ${entry}`);
    }
  }

  await mkdir(unpackPath);
  await run('tar', ['-xzf', archivePath, '-C', unpackPath, '--no-same-owner']);
  await stat(join(unpackPath, 'baerscript_wasm.js'));
  await stat(join(unpackPath, 'baerscript_wasm_bg.wasm'));
  await writeFile(
    join(unpackPath, '.baerscript-bundle.json'),
  ...
  console.log(`Staged BaerScript at ${outputPath}.`);
} finally {
