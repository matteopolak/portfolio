...
A project card may also receive a custom action descriptor. The descriptor renders a red button with a stable action ID; `src/lib/project-actions.ts` maps that ID to a client callback after direct entry and every Astro navigation. Minecraft's `Try in browser` callback opens `ProjectDemoModal.astro`, creates the shared `<lodestone-game>` element only on demand, and immediately starts its asset preparation while showing progress. It destroys the game element when the modal closes so hidden game audio and processing cannot continue. The reusable blog embed retains a `Try in browser` control so merely reading the post cannot initiate the 37.4 MiB download. Quasi uses the same project-card action and floating-dialog pattern for its unlabelled two-panel interpreter playground, with Run anchored inside the source pane. Both use a crisp, shadowless 16:9 surface over a 30%-dimmed, blurred page backdrop and a shared aggregate loader built by the site's existing Bauhaus pattern generator. Three square 3-by-3 compositions cycle in place: their nine cells fade in diagonally, hold, fade out, and give way to the next generated composition. Prepared code-playground workers remain idle and reusable after the dialog closes, so reopening a cached demo skips the loading surface; Astro navigation still destroys them. Opening uses a short scale-and-fade entrance; close controls, backdrop clicks, and Escape use the matching animated exit. Both transitions are disabled for reduced-motion visitors. Shared modal input handling consumes wheel, touch, and keyboard scrolling before it can move the page underneath, without changing the document's layout or scroll state. Explicit `data-project-demo-scroll` regions remain internally scrollable and contain overscroll at their edges; Quasi uses these for its source editor and output console. The Lodestone canvas receives focus as soon as the modal opens and again when its first frame is exposed. The loading surface is held until both the first frame and worker session are ready, after which the host synchronizes renderer focus. Lodestone currently requires WebGPU through a transferred `OffscreenCanvas` inside its render worker. Before requesting the SDK payload, the host checks page WebGPU, canvas transfer, workers, worker WebGPU, worker `OffscreenCanvas`, and creation of a worker WebGPU canvas context. An unsupported browser receives the exact missing capabilities with API names rendered as inline code; the unusable full-screen action is hidden with the error state. The current release receives a harmless `ArrowLeft` press before reveal so its already-painted ownership gate reconciles to the same internal screen instead of consuming the user's first click; this is a no-op once that upstream state is already synchronized. Every pointer-button event then repeats focus synchronization and sends the current canvas coordinates before the button state.
