...
Internal links use Astro's client router and explicitly request `push` history entries, so browser Back and Forward navigation retain each visited page even when the router's automatic history heuristic differs between production browsers. The single `.latent-grid` element is marked with `transition:persist`. Astro therefore moves the existing live grid DOM into the next page instead of recreating it: generated motifs, animation directions, generator sequence, active timers, and recently hovered ambient cells all remain in memory. No grid state is written to browser storage. Each full document load mixes fresh entropy into the generator-version seed, producing a new composition; client-side route changes preserve that live composition. After every client-side page load, special status is recomputed from the destination page's placeholders and collision map, allowing the same artwork to carry over without retaining stale layout assumptions.

Astro is configured with `output: 'static'` and file-format builds, so every route is emitted as a complete HTML document and can be served without an application server. The client router progressively enhances those same links by fetching and swapping the destination document during in-site navigation; direct entry, refresh, and no-JavaScript access continue to use the prerendered page.

The oversized homepage name is optically aligned to the content edge with a `-0.09em` CSS translation derived from the stable left side-bearing of Open Sans. Because the correction scales with the responsive font size and is present in the initial stylesheet, the heading does not visibly shift after paint. The site self-hosts the Latin Open Sans variable WOFF2 from `@fontsource-variable/open-sans`; Vite emits it as a same-origin hashed asset, and `Head.astro` preloads that exact built URL. The local TTF files under `resume/fonts/` remain isolated to Typst. Do not replace the CSS correction with post-paint canvas measurement or an external font service.

`src/assets/favicon.svg` is the only authored favicon source and contains the current solid-ink M. `pnpm generate:favicons` uses Sharp to derive the PNG, ICO, Apple touch, and WebP files; `predev` and `prebuild` run it automatically before the site consumes them. Generated variants are ignored by Git and are written only when their bytes change. The SVG and generated raster imports pass through Vite, which content-hashes their deployed URLs automatically so mobile icon caches cannot retain an older design. To change the mark, edit the SVG and restart the development server or run the generator directly—there are no variant filenames or manifests to update by hand.
...
The homepage, Blog, and Projects use a shared thick horizontal bar above the page title instead of numeric index tiles. The bar matches each route's navigation dot—red, blue, and yellow respectively—and uses the shared responsive `--page-title-gap` token. The non-home introductions use a deeper responsive top margin so their title groups share the homepage hero's lower visual starting position. Blog rows retain the shared editorial-list treatment. Projects remain full-width centered cards in a newest-first journey and retain their separation on hover with a neutral light-gray fill rather than the warm page color. Compact date-and-title milestones alternate around a straight light-gray rail that touches the neighboring project cards and exists only when events separate them. Small category-colored dots carry the only category styling. At the mobile breakpoint event text moves to one side of a short rail while project cards retain the full content width. Project cards put their textual AI tier and meter at the upper-right of the title row, ensuring the meaning does not depend on color. The title link stretches across the card; the footer passes pointer input through to it everywhere except the independent Website and GitHub action chips. See [project-journey.md](./project-journey.md) for the data and rendering flow. Article Markdown is styled through the global `.prose` class, while the table of contents updates its active entry and reading-progress bar with a small page script.
